        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shebeijiechu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/shiyanshipaike/upload/shebeijiechuMuBan.xls"
                        >批量导入设备借出数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shebeijiechu','导入导出')"
                                ...
                                :on-success="shebeijiechuUploadSuccess"
                                :on-error="shebeijiechuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shebeijiechu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入设备借出数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shebeijiechu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shebeijiechu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shebeijiechuNumber"
                                      header-align="center"
                                      label="借出数量">
                        <template slot-scope="scope">
                            {{scope.row.shebeijiechuNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('shebeijiechu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shebeijiechu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shebeijiechu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                shebeiId : null,
                xueshengId : null,
                shebeijiechuNumber : null,
                shebeijiechuContent : null,
                jiechuTime : null,
                guihuanTime : null,
                insertTime : null,
                createTime : null,
        ...
        methods: {
            chartDialog() {
                ...
                //         groupColumn: "shebeijiechu_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.shebeijiechuNumberStartSearch != '' && this.searchForm.shebeijiechuNumberStartSearch != undefined) {
                    params['shebeijiechuNumberStart'] = this.searchForm.shebeijiechuNumberStartSearch
                }
                if (this.searchForm.shebeijiechuNumberEndSearch != '' && this.searchForm.shebeijiechuNumberEndSearch != undefined) {
                    params['shebeijiechuNumberEnd'] = this.searchForm.shebeijiechuNumberEndSearch
                }
                                        
                params['shebeijiechuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "shebeijiechu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            shebeijiechuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shebeijiechu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
